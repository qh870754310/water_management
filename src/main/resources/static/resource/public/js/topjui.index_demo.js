...
$(function () {
    ...
    $(".systemName").on("click", function (e) {
        ...
        var title = $(this).data("title") + '闂ㄦ埛', url = $(this).data("url");
        if (url != undefined) {
            if (url.replace(" ", "") != "") {
                // 鏇存柊閫夋嫨鐨勯潰鏉跨殑鏂版爣棰樺拰鍐呭
                var tab = $('#index_tabs').iTabs('getTab', 0); // 鑾峰彇閫夋嫨鐨勯潰鏉�
                $('#index_tabs').iTabs('update', {
                    tab: tab,
                    options: {
                        title: title,
                        //href: $(this).data("url")  // 鏂板唴瀹圭殑URL
